...
If pTickSize = OneThirtySecond Then
    gFormatPrice = gFormatPriceAs32nds(pPrice)
ElseIf pTickSize = OneSixtyFourth Then
    If pSecType = SecTypeFuture Then
        gFormatPrice = gFormatPriceAs32ndsAndFractions(pPrice)
    Else
        gFormatPrice = gFormatPriceAs64ths(pPrice)
    End If
ElseIf pTickSize = OneHundredTwentyEighth Then
    If pSecType = SecTypeFuture Then
        gFormatPrice = gFormatPriceAs32ndsAndFractions(pPrice)
    Else
        gFormatPrice = gFormatPriceAs64thsAndFractions(pPrice)
    End If
Else
    gFormatPrice = gFormatPriceAsDecimals(pPrice, pTickSize)
End If
